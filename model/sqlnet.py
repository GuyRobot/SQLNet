...
from SQLNet.model.agg_predictor import AggPredictor
from SQLNet.model.sel_predictor import SelPredictor
from SQLNet.model.sqlnet_condition_predict import SQLNetCondPred
from SQLNet.model.word_embedding import WordEmbedding

import tensorflow as tf
import tensorflow.keras.layers as layers


class SQLNet(tf.keras.Model):
    def __init__(self, word_emb, N_word, N_h=100, N_depth=2,
        ...
        # Word embedding
        if trainable_emb:
            self.agg_embed_layer = WordEmbedding(word_emb, N_word,
                                                 self.SQL_TOK, our_model=True, trainable=trainable_emb)
            self.sel_embed_layer = WordEmbedding(word_emb, N_word,
                                                 self.SQL_TOK, our_model=True, trainable=trainable_emb)
            self.cond_embed_layer = WordEmbedding(word_emb, N_word,
                                                  self.SQL_TOK, our_model=True, trainable=trainable_emb)
        else:
        ...
        self.cond_pred = SQLNetCondPred(N_word, N_h, N_depth,
                                        self.max_col_num, self.max_tok_num, use_ca)

        self.CE = tf.keras.losses.CategoricalCrossentropy()
        self.softmax = layers.Softmax()
        self.log_softmax = tf.nn.log_softmax
        self.bce_logit = tf.keras.losses.BinaryCrossentropy(from_logits=True)
    ...
    def loss(self, score, truth_num, pred_entry, gt_where):
        ...
        if pred_agg:
            ...
            data = tf.convert_to_tensor(np.array(agg_truth))
            agg_truth_var = tf.Variable(data)

            loss += self.CE(agg_score, agg_truth_var)

        if pred_sel:
            sel_truth = map(lambda x: x[1], truth_num)
            data = tf.convert_to_tensor(np.array(sel_truth))
            sel_truth_var = tf.Variable(data)
        ...
        if pred_cond:
            ...
            data = tf.convert_to_tensor(np.array(cond_num_truth))
            cond_num_truth_var = tf.Variable(data)
            ...
            data = tf.convert_to_tensor(truth_prob)
            cond_col_truth_var = tf.Variable(data)

            sigm = layers.Activation("sigmoid")
            cond_col_prob = sigm(cond_col_score)
            bce_loss = -tf.reduce_mean(3 * (cond_col_truth_var *
                                            tf.math.log(cond_col_prob + 1e-10)) +
                                       (1 - cond_col_truth_var) * tf.math.log(1 - cond_col_prob + 1e-10))
            loss += bce_loss
            ...
            for b in range(len(truth_num)):
                if len(truth_num[b][4]) == 0:
                    continue
                data = tf.convert_to_tensor(np.array(truth_num[b][4]))
                cond_op_truth_var = tf.Variable(data)
            ...
            for b in range(len(gt_where)):
                for idx in range(len(gt_where[b])):
                    ...
                    if len(cond_str_truth) == 1:
                        continue
                    data = tf.convert_to_tensor(np.array(cond_str_truth[1:]))
                    cond_str_truth_var = tf.Variable(data)
