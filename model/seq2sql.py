...
from SQLNet.model.seq2sql_condition_predict import Seq2SQLCondPred
...
class Seq2SQL(tf.keras.Model):
    def __init__(self, word_emb, N_word, N_h=100, N_depth=2, trainable_emb=False):
        ...
        self.cond_pred = Seq2SQLCondPred(
    ...
    def loss(self, score, truth_num, pred_entry, gt_where):
        ...
        if pred_cond:
            for b in range(len(gt_where)):
                cond_truth_var = tf.Variable(
                    tf.convert_to_tensor(np.array(gt_where[b][1:])))
    ...
    def reinforce_backward(self, score, rewards):
        ...
        for t in range(len(cond_score[1])):
            reward_inp = tf.expand_dims(tf.convert_to_tensor(cur_reward), axis=1)
