...
class WordEmbedding(tf.keras.Model):
    ...
    def __init__(self, word_emb, N_word, SQL_TOK,
        ...
        if trainable:
            ...
            self.embedding = layers.Embedding(len(self.w2i), N_word)
            self.embedding.weight = tf.Variable(
                tf.convert_to_tensor(word_emb_val.astype(np.float32)))
        else:
    ...
    def gen_x_batch(self, q, col):
        ...
        if self.trainable:
            ...
            val_tok = tf.convert_to_tensor(val_tok_array)
            val_tok_var = tf.Variable(val_tok)
            ...
            val_inp = tf.convert_to_tensor(val_emb_array)
            val_inp_var = tf.Variable(val_inp)
